...
const styles = {
  ...
  code_block: "background:#1e1e1e;color:#d4d4d4;padding:16px;border-radius:8px;font-size:13px;line-height:1.6;margin:15px 0;font-family:Consolas,Monaco,monospace;white-space:pre-wrap;word-break:break-all;",
...
renderer.code = function ({ text, lang }) {
  const escaped = text
    ...
    .replace(/>/g, "&gt;")
    .replace(/\n/g, "<br>");
  return `<section style="${styles.code_block}"><code style="color:#d4d4d4;font-family:Consolas,Monaco,monospace;">${escaped}</code></section>`;
};
...
// 转换为 HTML（直接传 renderer 参数，避免 setOptions 的潜在问题）
const html = marked.parse(markdownWithoutH1, { renderer });

// 包装成完整的 HTML
const fullHtml = `<section style="max-width:100%;margin:0 auto;padding:16px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
${html}
</section>`;
